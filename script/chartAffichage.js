...
async function loadLightData(timeRange) {
	try {
		...
		console.error("Erreur lors du chargement des données de luminosité:", error)
...
function updateRecommendations() {
	...
	const currentHumidity =
		humidityData.length > 0 ? humidityData[humidityData.length - 1].value : null
	const currentLight =
		lightData.length > 0 ? lightData[lightData.length - 1].value : null
...
function updateDPV() {
	...
	if (temperatureData.length > 0 && humidityData.length > 0) {
		const currentTemperature = temperatureData[temperatureData.length - 1].value
...
function updateTrends() {
	...
	if (temperatureData.length >= 2) {
		...
		if (trendElement) {
			trendElement.textContent = `${trend >= 0 ? "+" : ""}${trend.toFixed(1)}°C`
