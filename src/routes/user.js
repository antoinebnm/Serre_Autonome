...
const userValidationRules = [
	check("prenom")
		...
		.isLength({ max: 50 })
		.withMessage("Prenom too long."),
	check("nom")
		.notEmpty()
		.withMessage("Nom is required.")
		.isLength({ max: 50 })
		.withMessage("Nom too long."),
	check("email")
		.notEmpty()
...
router.route("/api/v1/users/logout").post(authMiddleware, logout)

router.route("/api/v1/users/authentication").post(login)

// Route protégée spécifique - doit être définie AVANT les routes avec paramètres
